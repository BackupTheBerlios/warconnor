...
import java.util.*;
public class BmaCondizioni extends BmaServizio {
/**
 * BmaListaEntiEmittenti constructor comment.
 */
public BmaCondizioni() {
	super();
}
public String esegui(BmaJdbcTrx trx) throws BmaException {
	...
	if (tipoAzione.equals(BMA_SQL_SELECT)) {
		if (nomeTabella.equals("PD_CONDIZIONI")) {
			BmaDataColumn c = tabella.getColonna("COD_TIPOCONDIZIONE");
			c.setTipoControllo(BMA_CONTROLLO_LISTA);
			c.setValoriControllo(gp.valoriTipiCondizione(trx, input.getSocieta()).getStringTable());
		}
		else if (nomeTabella.equals("PD_PRODCOND")) {
			BmaDataColumn c = tabella.getColonna("COD_CONDIZIONE");
			c.setTipoControllo(BMA_CONTROLLO_LISTA);
			c.setValoriControllo(gp.valoriCondizioniGenerali(trx, input.getSocieta()).getStringTable());
		} 
		else {
			BmaDataColumn c = tabella.getColonna("COD_CONDIZIONE");
			c.setTipoControllo(BMA_CONTROLLO_LISTA);
	...
	/*
	else if (tipoAzione.equals(BMA_SQL_CLONE)) {
		gp.clonaCondizione(trx, tabella, filtri, new BmaJsp().getData());
		return "";
	}
	*/
	else {
