...
/**
 * BmaDataModel constructor comment.
 */
...
public String esegui(BmaJdbcTrx trx) throws BmaException {
	...
	String targetSource = input.getInfoServizio("targetSource");
	replayMode = input.getInfoServizio("replayMode");
	checkOnly = input.getInfoServizio("checkOnly");
...
/*
			compareTable(trx, trxTarget, jModel.getDataTable("PD_PRODOTTI"));
*/
...
private void loadNames() {
	BmaVector loadNames = jModel.getLoadOrder();
	for (int i = 0; i < loadNames.getSize(); i++){
		BmaVector level = (BmaVector)loadNames.getElement(i);
		for (int j = 0; j < level.getSize(); j++){
			tableNames.add(level.getVector().elementAt(j));
		}
	}
}
private String tableList() {
	...
	Arrays.sort(list);
	BmaVector loadNames = new BmaVector("tablesNames");
	for (int i = 0; i < list.length; i++){
		String s = (String)list[i];
		loadNames.setString(s, s);
	}
	return loadNames.toXml();
}
}
