...
public BmaVector getLoadOrder() {
	...
	while (bLoop) {
		...
		JdbcTableList lisTabelle = new JdbcTableList("Level-" + codLivello);
		bLoop = false;
		...
		while (e.hasMoreElements()) {
			...
			if (sTemp.indexOf("|" + jTable.tableName + "|") < 0) {
				if (jTable.importedKeys.size()==0) {
					...
					prog++;
					lisTabelle.getTableList().add(new BmaParametro(getNumeroXChiave(prog, 5), jTable.tableName));
				}
				else {
					boolean bLoad = true;
